trigger DupLOIUnitPeriod on LOI_Unit__c (before insert,before update) {
    ...
    if(untIds.size()>0){
        Map<Id,LOI__c> loiMap = new Map<Id,LOI__c>([Select id,LOI_Start_Date__c,LOI_Status__c,LOI_End_Date__c from LOI__c where id in:loiIds]);
        Map<Id,Units__c> unitMap = new Map<Id,Units__c>([Select id,Start_Date__c,End_Date__c from Units__c where id in:untIds]);
        
        List<LOI_Unit__c> loiUntList = [Select id,Name,Unit__c,LOI_Start_Date__c,LOI_End_Date__c from LOI_Unit__c where Unit__c in:untIds and LOI_Agreement_No__r.LOI_Status__c !='Expired' and LOI_Agreement_No__r.LOI_Status__c !='Cancelled'];
        ...
        if(loiUntList.size()>0){
            ...
            for(LOI_Unit__c lu:Trigger.New){
            	...
            	if(loiMap.get(lu.LOI_Agreement_No__c).LOI_Status__c != 'Cancelled' && loiMap.get(lu.LOI_Agreement_No__c).LOI_Status__c != 'Expired')
