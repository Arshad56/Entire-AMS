/*
 *      Name            :       POPaymentArrangementPreventDelete
 *      Author          :       Mohd Anas
 *      Date            :       7th July 2017
...
*/
trigger POPaymentArrangementPreventDelete on PO_Payment_Arrangement__c (before delete,before Insert,before Update) {
    ...
    */
    if(Trigger.isBefore && !Trigger.isDelete){
        ...
        if(Trigger.isInsert){
            ...
            for(AggregateResult ar : [SELECT Purchase_order__c,sum(payment_term__c) total
                                      FROM PO_Payment_Arrangement__c 
                                      WHERE Purchase_order__c in :poIdSet 
            ...
            for(AggregateResult ar : [SELECT Purchase_order__c,sum(payment_term__c) total
                                      FROM PO_Payment_Arrangement__c 
                                      WHERE Purchase_order__c in :poIdSet AND id not in :trigger.New 
                                      GROUP BY Purchase_order__c])
                sumOfPayTermOnvendor.put((Id)ar.get('Purchase_order__c'),(Decimal)ar.get('total'));
            system.debug('sumOfPayTermOnvendor>>>>>>>>>>>>>>>>>'+sumOfPayTermOnvendor);
