                    ...
                    if(rentrollToDetailMap.containsKey(rr.Id)){
                        ...
                        for(Id leaseId:tempMap.keyset()){
                            ...
                            if(rr.Billing_Type__c!='Late Charge'){
                                ...
                                System.debug('Exchange Rate Assignment in Approve RR \n Inv Exchange Rate '+inv.Exchange_Rate__c);
                                System.debug('Inv Foreign_Exchange_Rate__c '+inv.Foreign_Exchange_Rate__c);
                                ...
                                for(string str:tempMap2.keyset()){
                                    ...
                                    for(Rent_Roll_Detail__c rrd:tempMap2.get(str)){
                                        ...
                                        System.debug('rrd 11-->'+rrd.Id +' '+ listRRDRents.containskey(rrd.Id));
                                        System.debug('rrd 111-->'+ (invWrap.inv.Comm_Tax__c == null || invWrap.inv.Comm_Tax__c == 0) +' '+ rrd.Comm_Tax_Amount__c);
                                        if(listRRDRents.containskey(rrd.Id) && rrd.Comm_Tax_Amount__c != null && rrd.Comm_Tax_Amount__c > 0 && (invWrap.inv.Comm_Tax__c == null || invWrap.inv.Comm_Tax__c == 0)){
                                        	System.debug('listRRDRents.get(rrd.Id)-->'+ listRRDRents.get(rrd.Id));
                                        	for(Rent_Schedules__c rs:listRRDRents.get(rrd.Id)){
                                        		System.debug('rs.Comm_Tax__c-->'+ rs.Comm_Tax__c);
	                                        	if(rs.Comm_Tax__c != null && rs.Comm_Tax__c > 0 ){
	                                        		invWrap.inv.Comm_Tax__c = rs.Comm_Tax__c;
	                                        	}
	                                        }
                                        }
                                    ...
                                    for(Rent_Roll_Detail__c rrd:tempMap2.get(str)){
                                        ...
                                        if(listRRDRents.containskey(rrd.Id) && rrd.Comm_Tax_Amount__c != null && rrd.Comm_Tax_Amount__c > 0 && (invWrap.inv.Comm_Tax__c == null || invWrap.inv.Comm_Tax__c == 0)){
                                        	...
                                        	for(Rent_Schedules__c rs:listRRDRents.get(rrd.Id)){
                                        		System.debug('rs.Comm_Tax__c-->'+ rs.Comm_Tax__c);
	                                        	if(rs.Comm_Tax__c != null && rs.Comm_Tax__c > 0 ){
	                                        		inv.Comm_Tax__c = rs.Comm_Tax__c;
	                                        	}
	                                        }
                                        }
